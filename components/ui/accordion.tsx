...
>(({ className, ...props }, ref) => (
    <AccordionPrimitive.Item
        ref={ref}
        className={cn(
            "border-border bg-card rounded-3xl mb-2 px-6 shadow-sm hover:shadow-md transition-all duration-200",
            className
        )}
        {...props}
    />
...
>(({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header className="flex">
        <AccordionPrimitive.Trigger
            ref={ref}
            className={cn(
                "flex flex-1 items-center justify-between py-4 font-medium text-foreground transition-all hover:text-muted-foreground [&[data-state=open]>svg]:rotate-180",
                className
            )}
            {...props}
        >
